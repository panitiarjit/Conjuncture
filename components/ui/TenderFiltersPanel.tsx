...
export default function TenderFiltersPanel({ state }: Props) {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 flex-shrink-0 gap-5 sticky top-20" aria-label="Tender filters">
      ...
      <FilterSection title={t('tp.region')}>
        <select value={location} onChange={(e) => setLocation(e.target.value)} className="input text-sm" aria-label={t('tp.region')}>
